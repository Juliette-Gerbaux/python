...
def find_thermal_prod_var(solver):

    var = solver.variables()
    var_id = [
        i for i in range(len(var)) if re.search("DispatchableProduction", var[i].name())
    ]
...
def get_thermal_ini_file(dir_study: str, area: str) -> ConfigParser:
    ini_file = ConfigParser()
    ini_file.read(dir_study + "/input/thermal/clusters/" + area + "/list.ini")
